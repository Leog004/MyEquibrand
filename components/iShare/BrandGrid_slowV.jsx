...
export default function BrandGrid_New({title, handleChange}) {
    ...
    async function download() {
       ...
       let folder = zip.folder('collection');  // creates a folder
        ...
        const data = await fetch('/api/readerZip_new?' + new URLSearchParams({
            brand: title,
        ...
        data.files.map((el, index) => {
            ...
            const imageBlob = fetch(`data:image/png;base64,${el.value}`).then(res => res.blob())   
            // creating a file to our zip folder. Parameter 1: filename 'exampleFile.png', Parameter 2: blob
            folder.file(`${el.fileName}`, imageBlob); 
